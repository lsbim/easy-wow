...
import { TL_DURATION_RECT_HEIGHT, TL_Y_BOSS_TEXT, TL_Y_PER_LIST } from "../../../global/variable/timelineConstants";
import ImageCanvas from "./ImageCanvas";

const BossCastCanvas = ({ enemyCastsTimeline, handleMouseEnter, handleMouseLeave, firstBoss, combatTime, selectedBossSkill, timelineScaleX }) => {
    ...
    return (
        <>
            ...
            <Group>
                ...
                <Rect
                    y={(TL_Y_PER_LIST) + 0.5}
                    width={combatTime / 1000 * timelineScaleX}
                ...
                {/* 보스 전투시간 텍스트 */}
                <Text
                    x={combatTime / 1000 * timelineScaleX + 10}
                    y={TL_Y_BOSS_TEXT}
                ...
                {/* 보스 캐스팅 박스, 텍스트 */}
                <Group
                    clipWidth={combatTime / 1000 * timelineScaleX}
                    clipHeight={1000}>
                    {enemyCastsTimeline
                        ...
                        ?.filter(c => selectedBossSkill?.has(c?.abilityGameID))?.map((cast, enemyCastIndex) => {
                            ...
                            return (
                                <React.Fragment key={enemyCastIndex + 'cast'}>
                                    <Rect
                                        x={timestampToPosition(cast.timestamp - firstBoss.startTime, timelineScaleX)}
                                        y={TL_Y_PER_LIST + 1}
                                    ...
                                    />
                                    <Text
                                        x={timestampToPosition(cast.timestamp - firstBoss.startTime, timelineScaleX) + 30}
                                        y={TL_Y_BOSS_TEXT}
