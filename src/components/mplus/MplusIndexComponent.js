...
const MplusIndexComponent = () => {
    ...
    const [selected, setSelected] = useState({ dungeon: 0, spec: '' })
    ...
    useEffect(() => {
        if (selected.dungeon !== 0 && selected.spec !== '') {
            navigate(`/mplus/${selected.dungeon}/${selected.spec}`);
        }
    }, [selected])
    ...
    return (
        <div>
            ...
            <div className="flex mt-10">
                {wowClassList.map(c => (
                    <div className="flex flex-col items-center ml-2" key={c?.name}>
                        <img
                        ...
                        <div className="">
                            {c?.specs?.filter(s => s.has).map(s => (
                                <img
                                    ...
                                    alt={s?.name}
                                    title={s?.name}
                                    onClick={() => { s?.has && handleSpecClick(c?.name + '-' + s?.name) }}
                                />
                            ))}
                        </div>
                    </div>
